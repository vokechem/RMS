...
class AttestationApproval extends Component {
    constructor() {
        super();
        this.state = {
            open: false,
    ...
    fetchMyApplications = () => {
    fetch("/api/AttestationApproval/" + localStorage.getItem("UserName"),{
      method: "GET",
      headers: {
      ...
      .then(Attestation => {
        if (Attestation.length > 0) {
          ...
          swal("", Attestation.message, "error");
        }
      })
      .catch(err => {
    ...
    handleSubmit = event => {
        ...
        if (this.state.IsAccept){
            this.Approve("/api/AttestationApproval", data);
        }
        if (this.state.IsDecline) {
            this.Decline("/api/AttestationApproval", data);
        }
  ...
  notifyPanelmembers = (AproverMobile, Name, AproverEmail, IDNumber, Msg) => {        
    if (Msg == "Applicant") {         
        this.SendSMS(
            AproverMobile,
            "Dear " +Name + " Attestation that you had submited to Job majuu has been Approved."
        );
        this.SendMail(
            Name,
            AproverEmail,
...
Approve(url = ``, data = {}) {
      ...
      .then(response =>
          response.json().then(data => {                   
              ...
              if (data.success) {                   
                  ...
                  swal("", "Attestation Approved", "success");
  ...
  sendDeclineNotification = (Mobile, Name, Email, IDNumber)=>{
      this.SendSMS(
          Mobile,
          "Dear " +Name + " Attestation that you had submited to Job majuu  has been declined."
      );
      this.SendMail(
          Name,
          Email,
          ...
          .then(response =>
              response.json().then(data => {
                  ...
                  if (data.success) {                      
                      ...
                      swal("", "Attestation Declined", "success"); 
                      this.fetchMyApplications();        
                        ...
                        <div class="content-header row">
                            <div class="content-header-left col-md-9 col-12 mb-2">
                                <div class="row breadcrumbs-top">
                                    <div class="col-12">
                                        <h4 class="content-header-title float-left mb-0">Pending NEA Approval</h4>
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default AttestationApproval;
