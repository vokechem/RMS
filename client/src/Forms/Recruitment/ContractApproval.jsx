...
class ContractApproval extends Component {
    ...
    fetchMyApplications = () => {
    fetch("/api/ContractApproval/" + localStorage.getItem("UserName"),{
      method: "GET",
      headers: {
    ...
    handleSubmit = event => {
        ...
        if (this.state.IsAccept){
            this.Approve("/api/ContractApproval", data);
        }
        if (this.state.IsDecline) {
            this.Decline("/api/ContractApproval", data);
        }
  ...
  notifyPanelmembers = (AproverMobile, Name, AproverEmail, IDNumber, Msg) => {        
    if (Msg == "Applicant") {         
        this.SendSMS(
            AproverMobile,
            "Dear " +Name + " Contract processing that you had submited to Job majuu has been Approved."
        );
        this.SendMail(
            Name,
            AproverEmail,
            "Contract Approval",
            "Contract Processing Approved",
            IDNumber,
...
Approve(url = ``, data = {}) {
  ...
  sendDeclineNotification = (Mobile, Name, Email, IDNumber)=>{
      this.SendSMS(
          Mobile,
          "Dear " +Name + " Contract processing that you had submited to Job majuu  has been declined."
      );
      this.SendMail(
          Name,
          Email,
          "Contract Declined",
          "Contract Processing Declined",
          IDNumber,
          this.state.Remarks
      );
  }
          ...
          .then(response =>
              response.json().then(data => {
                  ...
                  if (data.success) {                      
                      ...
                      swal("", "Contract processing Declined", "success"); 
                      this.fetchMyApplications();        
                        ...
                        <div class="content-header row">
                            <div class="content-header-left col-md-9 col-12 mb-2">
                                <div class="row breadcrumbs-top">
                                    <div class="col-12">
                                        <h4 class="content-header-title float-left mb-0">Pending Contract Approval</h4>
                                    </div>
                                </div>
                            </div>
                        </div>
