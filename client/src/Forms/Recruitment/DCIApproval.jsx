...
class DCIApproval extends Component {
    constructor() {
        super();
        this.state = {
            open: false,
    ...
    fetchMyApplications = () => {
    fetch("/api/DCIApproval/" + localStorage.getItem("UserName"),{
      method: "GET",
      headers: {
    ...
    handleSubmit = event => {
        ...
        if (this.state.IsAccept){
            this.Approve("/api/DCIApproval", data);
        }
        if (this.state.IsDecline) {
            this.Decline("/api/DCIApproval", data);
        }
  ...
  notifyPanelmembers = (AproverMobile, Name, AproverEmail, IDNumber, Msg) => {        
    if (Msg == "Applicant") {         
        this.SendSMS(
            AproverMobile,
            "Dear " +Name + " DCI Clearance that you had submited to Job majuu has been Approved."
        );
        this.SendMail(
            Name,
            AproverEmail,
...
Approve(url = ``, data = {}) {
      ...
      .then(response =>
          response.json().then(data => {                   
              ...
              if (data.success) {                   
                  ...
                  swal("", "DCI Clearance Approved", "success");
  ...
  sendDeclineNotification = (Mobile, Name, Email, IDNumber)=>{
      this.SendSMS(
          Mobile,
          "Dear " +Name + " DCI Clearance that you had submited to Job majuu  has been declined."
      );
      this.SendMail(
          Name,
          Email,
          ...
          .then(response =>
              response.json().then(data => {
                  ...
                  if (data.success) {                      
                      ...
                      swal("", "DCI Clearance Declined", "success"); 
                      this.fetchMyApplications();        
