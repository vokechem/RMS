...
class MinormedicalApproval extends Component {
    constructor() {
        super();
        this.state = {
            open: false,
    ...
    fetchMyApplications = () => {
    fetch("/api/MinorApproval/" + localStorage.getItem("UserName"),{
      method: "GET",
      headers: {
    ...
    handleSubmit = event => {
        ...
        if (this.state.IsAccept){
            this.Approve("/api/MinorApproval", data);
        }
        if (this.state.IsDecline) {
            this.Decline("/api/MinorApproval", data);
        }
  ...
  notifyPanelmembers = (AproverMobile, Name, AproverEmail, IDNumber, Msg) => {        
    if (Msg == "Applicant") {         
        this.SendSMS(
            AproverMobile,
            "Dear " +Name + " your minor medical result  has been Approved."
        );
        this.SendMail(
            Name,
            AproverEmail,
...
Approve(url = ``, data = {}) {
  fetch(url, {
    ...
    sendDeclineNotification = (Mobile, Name, Email, IDNumber)=>{
      this.SendSMS(
          Mobile,
          "Dear" +Name + " your Minor Medical Result has been declined."
      );
      this.SendMail(
          Name,
          Email,
          "MinorMedical Declined",
                        ...
                        <div class="content-header row">
                            <div class="content-header-left col-md-9 col-12 mb-2">
                                <div class="row breadcrumbs-top">
                                    <div class="col-12">
                                        <h4 class="content-header-title float-left mb-0">Pending Minor medical Approval</h4>
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default MinormedicalApproval;
