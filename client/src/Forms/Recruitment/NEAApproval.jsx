...
class NEAApproval extends Component {
    constructor() {
        super();
        this.state = {
            open: false,
    ...
    fetchMyApplications = () => {
    fetch("/api/NEAApproval/" + localStorage.getItem("UserName"),{
      method: "GET",
      headers: {
    ...
    handleSubmit = event => {
        ...
        if (this.state.IsAccept){
            this.Approve("/api/NEAApproval", data);
        }
        if (this.state.IsDecline) {
            this.Decline("/api/NEAApproval", data);
        }
  ...
  notifyPanelmembers = (AproverMobile, Name, AproverEmail, IDNumber, Msg) => {        
    if (Msg == "Applicant") {         
        this.SendSMS(
            AproverMobile,
            "Dear " +Name + " NEA that you had submited to Job majuu has been Approved."
        );
        this.SendMail(
            Name,
            AproverEmail,
...
Approve(url = ``, data = {}) {
  ...
  sendDeclineNotification = (Mobile, Name, Email, IDNumber)=>{
      this.SendSMS(
          Mobile,
          "Dear " +Name + " Major Medical that you had submited to Job majuu  has been declined."
      );
      this.SendMail(
          Name,
          Email,
