...
class PassportApproval extends Component {
  ...
  notifyPanelmembers = (AproverMobile, Name, AproverEmail, IDNumber, Msg) => {        
    if (Msg == "Applicant") {         
        this.SendSMS(
            AproverMobile,
            "Dear " +Name + " Passport Processing that you had submited to Job majuu has been Approved."
        );
        this.SendMail(
            Name,
            AproverEmail,
            "Passport Approval",
            "Passport Processing approved",
            IDNumber,
        );
    }
}
Approve(url = ``, data = {}) {
      ...
      .then(response =>
          response.json().then(data => {                   
              ...
              if (data.success) {                   
                  ...
                  swal("", "Passport Processing Approved", "success");
  ...
  sendDeclineNotification = (Mobile, Name, Email, IDNumber)=>{
      this.SendSMS(
          Mobile,
          "Dear " +Name + " Passport Processing that you had submited to Job majuu  has been declined."
      );
      this.SendMail(
          Name,
          Email,
          "Passport Declined",
          "Passport Processing Declined",
          IDNumber,
          this.state.Remarks
      );
  }
          ...
          .then(response =>
              response.json().then(data => {
                  ...
                  if (data.success) {                      
                      ...
                      swal("", "Passport Processing Declined", "success"); 
                      this.fetchMyApplications();        
