...
class TravelApproval extends Component {
    constructor() {
        super();
        this.state = {
            open: false,
    ...
    fetchMyApplications = () => {
    fetch("/api/TravellingApproval/" + localStorage.getItem("UserName"),{
      method: "GET",
      headers: {
    ...
    handleSubmit = event => {
        ...
        if (this.state.IsAccept){
            this.Approve("/api/TravellingApproval", data);
        }
        if (this.state.IsDecline) {
            this.Decline("/api/TravellingApproval", data);
        }
