...
class VisaApproval extends Component {
    constructor() {
        super();
        this.state = {
            open: false,
    ...
    fetchMyApplications = () => {
    fetch("/api/VisaApproval/" + localStorage.getItem("UserName"),{
      method: "GET",
      headers: {
    ...
    handleSubmit = event => {
        ...
        if (this.state.IsAccept){
            this.Approve("/api/VisaApproval", data);
        }
        if (this.state.IsDecline) {
            this.Decline("/api/VisaApproval", data);
        }
  ...
  notifyPanelmembers = (AproverMobile, Name, AproverEmail, IDNumber, Msg) => {        
    if (Msg == "Applicant") {         
        this.SendSMS(
            AproverMobile,
            "Dear " +Name + " Visa Processing that you had submited to Job majuu has been Approved."
        );
        this.SendMail(
            Name,
            AproverEmail,
            "Visa Approval",
            "Visa Processing approved",
            IDNumber,
...
Approve(url = ``, data = {}) {
      ...
      .then(response =>
          response.json().then(data => {                   
              ...
              if (data.success) {                   
                  ...
                  swal("", "Visa Processing Approved", "success");
  ...
  sendDeclineNotification = (Mobile, Name, Email, IDNumber)=>{
      this.SendSMS(
          Mobile,
          "Dear " +Name + " Visa Processing that you had submited to Job majuu  has been declined."
      );
      this.SendMail(
          Name,
          Email,
          ...
          .then(response =>
              response.json().then(data => {
                  ...
                  if (data.success) {                      
                      ...
                      swal("", "Visa Processing Declined", "success"); 
                      this.fetchMyApplications();        
                        ...
                        <div class="content-header row">
                            <div class="content-header-left col-md-9 col-12 mb-2">
                                <div class="row breadcrumbs-top">
                                    <div class="col-12">
                                        <h4 class="content-header-title float-left mb-0">Pending Visa Approval</h4>
                                    </div>
                                </div>
                            </div>
                        </div>
